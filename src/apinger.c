...
void make_reports(struct target *t,struct alarm_cfg *a,int on,char* thisid,char* lastid) {
	...
	if (on>0) {
		command=a->command_on;		
	} else {
		command=a->command_off;
	}
...
void toggle_alarm(struct target *t,struct alarm_cfg *a,int on) {
	...
	if (a->combine_interval>0) {
		...
		if (tdr!=NULL && strcmp(tdr->t.name,t->name)==0 && tdr->a==a && tdr->on==on) {
			return;
		}
		...
		make_reports(t,a,on,thisid,lastid);
	}
	free(thisid);
	free(lastid);
}
...
void send_probe(struct target *t) {
	...
	if (t->addr.addr.sa_family==AF_INET) {
		send_icmp_probe(t,seq);
	}
#ifdef HAVE_IPV6
	else { 
		if (t->addr.addr.sa_family==AF_INET6) {
			send_icmp6_probe(t,seq);
		}
	}
#endif
...
void analyze_reply(struct timeval time_recv,int icmp_seq,struct trace_info *ti) {
	...
	if (ti->seq>t->last_received) {
		t->last_received=ti->seq;
	}
...
void receiver_loop(void) {
	...
	while (1) {
		...
		for (i=0;i<npfd;i++) {
			...
			if (pfd[i].fd==icmp_sock) {
				recv_icmp();
			} else {
				if (pfd[i].fd==icmp6_sock) {
					recv_icmp6();
				}
			}
